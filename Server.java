...
public class Server {
	...
	public static void main(String[] args) throws IOException {
		...
		ConcurrentHashMap broadQueue = new ConcurrentHashMap();
		if (args.length != 3) {
		...
		if (myNode.isRoot()) {
			...
			clientT.start();
			// System.out.println("The spinning tree has been built:
			// "+myNode.getIsTreeFinish());
			// Boolean a=false;
			// while (!myNode.getIsTreeFinish()){
			// System.out.println("The spinning tree has been built in main:
			// "+myNode.getIsTreeFinish());
			// }
			// System.out.println("The spinning tree has been built in main:
			// "+myNode.getIsTreeFinish());
	...
	public void run() {
		while (true) {
			try {
				...
				// newComingObj.getOrigin().getNodeId() + ", Message: "+
				// newComingObj.getType());

				if ((String) newComingObj.getType().intern() != ("SpinningBroad").intern()
					...
					if (!myNode.isParent() && !myNode.isRoot()) {
						System.out.println(
								"Replied received msg to: " + newComingObj.getOrigin().getNodeId() + ", Message: PACK");

						myNode.setParent(newComingObj.getOrigin());
						myNode.popParentFromNeighbors(newComingObj.getOrigin());
						// System.out.println("After pop neiborhood num" +
						// myNode.getNeighbors().size());
						...
						System.out.println(
								"Replied received msg to: " + newComingObj.getOrigin().getNodeId() + ", Message: NACK");
				...
				} else if ((String) newComingObj.getType().intern() == ("SpinningBroad").intern()) { // message type ==
																										// SpinningBroad
					myNode.setIsTreeFinish(true);
					System.out.println("---------------Start to sending spanning tree completion------------ ");
					System.out.println("Received msg from: " + newComingObj.getOrigin().getNodeId() + ", Message: "
							+ newComingObj.getType());
					...
					System.out.println("Received msg from: " + newComingObj.getDestination().getNodeId() + ", Message: "
							+ newComingObj.getType() + " For source Node" + newComingObj.getSource().getNodeId());
...
class AckThread extends Thread {
	...
	public void run() {
		...
		System.out.println("Send ACK to: " + this.msg.getOrigin().getNodeId());

		Node targetNode = this.msg.getOrigin();
...
class Broadcast extends Thread {
	...
	public void run() {
		...
		// where broadcast message only propagate to neighbors except the previous broadcast message sender
		for (int i = 0; i < this.myNode.getChildrenCnt(); i++) {
